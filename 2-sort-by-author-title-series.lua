-- 2-sort-by-author-title-series.lua
...
local function compareAuthorSeries(a, b, author_sort_type)
    ...
    -- 2. Build a "series key" that lets standalones sort by title
    local series_a = a.doc_props.series
    local series_b = b.doc_props.series

    local key_a = (series_a and series_a ~= "\u{FFFF}") and series_a or a.doc_props.display_title
    local key_b = (series_b and series_b ~= "\u{FFFF}") and series_b or b.doc_props.display_title

    -- 3. Compare series key (this merges standalones + series alphabetically)
    if key_a ~= key_b then
    ...
    -- 5. Final fallback: compare titles
